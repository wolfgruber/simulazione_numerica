...
// Esercizio 07.1: Monte Carlo Molecular Simulation
...
//update of the histogram of g(r)
     for (int i=0; i<nbins ; i++) {
       if ((dr < (i+1)*bin_size) and (dr >= i*bin_size)) {
         walker[i+igofr] += 2;
         break;  
       }
     }
...
void Averages(int iblk) //Print results for current block
{
   ...
   double r, gdir, vdir;
   ofstream Gofr, Gave, Gerr, Epot, Pres;
   const int wd=12;
    ...
    Gerr.open("output.gerr.0",ios::app);
...
//g(r)
    ...
    for (int i = 0; i < nbins; i++) {
        ...
        gdir = blk_av[i+igofr]/blk_norm/(rho * npart * vdir);
        ...
        Gofr << " " << gdir;
        Gave << " " << glob_av[i+igofr]/(double)iblk;
        Gerr << " " << err_gdir;
    }
